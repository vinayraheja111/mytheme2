...
function wp_bootstrap_starter_posted_on() {
	...
	if (!is_single())
		$posted_on = sprintf(
			esc_html_x( '%s', 'post date', 'dreamarina' ),
	...
	$byline = sprintf(
		esc_html_x( 'od %s', 'post author', 'dreamarina' ),
		'<span class="author vcard"><a class="url fn n" href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '">' . esc_html( get_the_author() ) . '</a></span>'
	);
	if (!is_single()){
		echo '<div class="row text-center"><div class="col-12">';
		echo '<span class="posted-on grid">' . $posted_on . '</span>'; // WPCS: XSS OK.<span class="byline"> ' . $byline . '</span>
		echo '</div></div>';
	}else{
		echo '<div class="row"><div class="col-12">';
		echo '<span class="posted-on">' . $posted_on . '</span> '.$byline; // WPCS: XSS OK.<span class="byline"> ' . $byline . '</span>
		echo '</div></div>';
	}
...
function wp_bootstrap_starter_entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		$categories_list = get_the_category_list( esc_html__( ' | ', 'dreamarina' ) );
		...
		$tags_list = get_the_tag_list( '', esc_html__( ', ', 'dreamarina' ) );
		if ( $tags_list ) 
			printf( '<span class="tags-links">' . esc_html__( 'Značky %1$s', 'dreamarina' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		echo '</div></div>';
	}
	edit_post_link(
		sprintf(
       ...
       // $GLOBALS['comment'] = $comment;
        ...
        if ( 'pingback' == $comment->comment_type || 'trackback' == $comment->comment_type ) : ?>
            ...
            <div class="comment-body">
                <?php _e( 'Pingback:', 'dreamarina' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( __( 'Edit', 'dreamarina' ), '<span class="edit-link">', '</span>' ); ?>
            </div>
                ...
                <div class="media-body">
                    <div class="media-body-wrap card">
                        ...
                        <div class="card-header">
                            <h5 class="mt-0"><?php printf( __( '%s <span class="says">says:</span>', 'dreamarina' ), sprintf( '<cite class="fn">%s</cite>', get_comment_author_link() ) ); ?></h5>
                            <div class="comment-meta">
                                <a href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ); ?>">
                                    <time datetime="<?php comment_time( 'c' ); ?>">
                                        <?php printf( _x( '%1$s at %2$s', '1: date, 2: time', 'dreamarina' ), get_comment_date(), get_comment_time() ); ?>
                                    </time>
                                </a>
                                <?php edit_comment_link( __( '<span style="margin-left: 5px;" class="glyphicon glyphicon-edit"></span> Edit', 'dreamarina' ), '<span class="edit-link">', '</span>' ); ?>
                            </div>
                        </div>

                        <?php if ( '0' == $comment->comment_approved ) : ?>
                            <p class="comment-awaiting-moderation"><?php _e( 'Your comment is awaiting moderation.', 'dreamarina' ); ?></p>
                        <?php endif; ?>
